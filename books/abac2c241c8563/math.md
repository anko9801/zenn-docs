...
ここでは基本的なアイデアだけを示すので厳密な証明や実装が気になった人は参考文献の本や規格書、元論文などを読んで実装してみると良いと思います。非本質的な部分は曖昧にして本質的な部分は出来るだけ厳密に書きます。何が厳密で何が厳密ではないかは明確に書くつもりなのでそこは安心してください。資料の厳密さはその情報の信頼度を表していると思っています。
...
> **位数発見問題**
> $a$ の位数 $n$ つまり $a^n = 1 \pmod N$ となる最小の $n$ を見つける。
