...
について解説します。楕円曲線は Fermat の最終定理を解く道具でもあったりする現代数学の要でもあるので流石に可換環論や代数幾何学を学んでいない読者は置いていくと思います。
...
まずは 2 変数のときグラフ上の
一次不定方程式
$ax + by = c$ は $\gcd(a, b)|c$ である

$(bm + k, -am + l)$


与えられる式が 1 つの 1 次方程式を LLL で解く方法については良記事があります。
...
「剰余の方程式は係数がある程度小さければそのまま整数方程式となるよ」という主張をしています。ここで勘のいい人はLLLを用いて係数を小さくすれば整数方程式に変換できて解けるのでは...！？と気付くでしょう。実際に考えてみましょう。
...
g_{0,1}^{(0)} & g_{0,1}^{(1)}X & g_{0,1}^{(2)}X^2 & \cdots & g_{0,1}^{(n)}X^n \\
&& \vdots \\
...
g_{2,0}^{(0)} & g_{2,0}^{(1)}X & g_{2,0}^{(2)}X^2 & \cdots & g_{2,0}^{(n)}X^n \\
&& \vdots \\
g_{2,l}^{(0)} & g_{2,l}^{(1)}X & g_{2,l}^{(2)}X^2 & \cdots & g_{2,l}^{(n)}X^n \\
&& \vdots \\
