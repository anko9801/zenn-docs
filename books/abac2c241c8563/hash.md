...
### CRC
ビット列を有限体を用いて短いビット列に変換するハッシュ関数です。これは誤り検出に用いられていて暗号学的ハッシュ関数ではないです。

次数 $n$ の生成多項式 $f(x)$ を用いて表される有限体 $\mathbb{F}_{2^n} \cong \mathbb{F}_2[x]/(f(x))$ において関数 $g: \mathbb{F}_2[x]/(f(x))\ni m \mapsto mx^n\in\mathbb{F}_2[x]/(f(x))$ をハッシュ関数とします。例えば入力が $1011000_{(2)}$ で生成多項式が $f(x) = x^4 + x + 1$ の場合、次の計算から出力は $1111_{(2)}$ となります。
