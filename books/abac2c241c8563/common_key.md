...
> **Prop.**
> ランダムなバイトについてパディングが適当となる確率は約 $1/255$ である。

**Proof.**
パディングとして適当となるには末尾のバイト列について考えればよい。

ランダムな文字列の最後のバイト列が `\x01` となる確率は $1/256$
ランダムな文字列の最後のバイト列が `\x02\x02` となる確率は $1/256^2$

これを繰り返すことで確率は次のようになる。

$$
\sum_{n = 1}^{16}\frac{1}{256^n} \approx \sum_{n = 0}^{\infty}\frac{1}{256^n} - 1 = \frac{1}{255}
$$

これより示された。$\Box$

$n+1$ 個目のブロックを鍵を知らずに解読することを目指します。

まず復号をおさらいします。`n+1` 個目のブロックを復号した結果は `P[n] = Dec_K(C[n]) ^ C[n-1]` となるのでした。

ここで `C[n-1]` だけ変更して他はそのままにした暗号文 `C'` を送ってパディングエラーとなるかどうかを考えます。

まず `C'[n-1]` が 16 バイトすべて `\x00` となるように `C'[n-1] = \x00 * 16` として `C'[n-1]` の最後の 1 バイトだけを `\x00` から `\xff` まで変えてすべて送ってみます。

そうするとパディングエラーとならないときが必ず 1 回は確実に出てきます。

これは何かというと、`C'` を復号した結果 `P'[n]` の最後の 1 バイトが `\x01` となる場合です。そうすると例えば `C'[n-1][-1] = \x12` であるとすると `P'[n][-1] = \x01` であるから `Dec(C[n])[-1] = P'[n][-1] ^ C'[n-1][-1] = \x01 ^ \x12 = \x13` と分かります！これより `P[n][-1] = Dec(C[n])[-1] ^ C[n-1][-1]` と計算すれば本当の平文の最後の 1 バイトが求まります！

TODO: 図

次に最後の 2 バイトが `\x02\x02` となるように調整します。

まず `C'[n-1][-1] = C'[n-1][-1] ^ \x01 ^ \x02` とすれば `P'[n]` の最後の 1 バイトが `\x02` となります。次にパディングエラーを用いて最後から 2 バイト目を `\x02` となるような値を見つけ出します。

上と同様に `C'[n-1][-2]` を `\x00` から `\xff` まで変えてすべて送ると `P'[n][-2] = \x02` となるときにパディングエラーが出ません。このときの値を使って `Dec(C[n])[-2] = P'[n][-2] ^ C'[n-1][-2]` と求めて `P[n][-2] = Dec(C[n])[-2] ^ C[n-1][-2]` と計算できます。

これを繰り返すことで 256 x 16 回だけ送れば鍵を一切知らずに平文 `P[n]` を求めることができます！

これには POODLE Attack (Padding Oracle On Downgraded Legacy Encryption) という名前が付いています。
