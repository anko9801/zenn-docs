...
次に `P'[n][-2:] = \x02\x02` となるように調整します。
まず `C'[n-1][-1] = C'[n-1][-1] ^ \x01 ^ \x02` とすると `P'[n][-1] = \x02` となります。
この状態で上と同様に `C'[n-1][-2]` を `\x00` から `\xff` まで変えてすべて送ると `P'[n][-2] = \x02` となるときにパディングエラーが出ません。このときの値を使って `Dec(C[n])[-2] = P'[n][-2] ^ C'[n-1][-2]` と求めて `P[n][-2] = Dec(C[n])[-2] ^ C[n-1][-2]` と計算できます。
