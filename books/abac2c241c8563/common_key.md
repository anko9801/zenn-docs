...
次に最後の 2 バイトが `\x02\x02` となるように調整して平文の 2 バイト目を求めます。

まず最後の 1 バイト目は `C'[n-1][-1] = C'[n-1][-1] ^ \x01 ^ \x02` とし、最後から 2 バイト目は `C'[n-1][-2]` を `\x00` から `\xff` まで変えてパディングエラーとならない値にすることで `P'[n][-2:] = \x02\x02` となります。このときの値を使って `Dec(C[n])[-2] = P'[n][-2] ^ C'[n-1][-2]` と求めて `P[n][-2] = Dec(C[n])[-2] ^ C[n-1][-2]` と計算できます。
