...
これは文字列を ASCII を用いて数にし、素数 $p, q$ を用いて $N = pq$ と $e$ で暗号化、$e^{-1}$ でします。
```
...
この機能って署名の機能と同じじゃないでしょうか。

署名というのはどういう性質をもつのか
自分であることを証明
手書きのサインや捺印で署名を行うがデジタルでは簡単にコピーできてしまう。

暗号文をそのまま改ざんされないように元の平文のハッシュ化して信頼できる第三者に秘密鍵で"暗号化"して貰えれば
