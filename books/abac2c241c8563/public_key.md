...
### ゼロ知識証明と電子署名
ゼロ知識という言葉は確率分布のシミュレーションパラダイムにおいてとは

安全性
- 情報理論的に安全
完全にランダムな情報は圧縮できない
暗号文のエントロピー (乱雑さ) が暗号文の長さと一致するとき安全とする立場。
- シミュレーションパラダイム
...
> 2. 検証者はチャレンジと呼ばれる乱数 $e$ を送る。
> 3. 証明者は $c = r + ex\bmod q$ を送る。
> 4. 検証者は $g^c = ay^e\bmod p$ となることを検証する。
