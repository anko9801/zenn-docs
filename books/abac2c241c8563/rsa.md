...
そして平文 $m$ に対して暗号文を $m^e \bmod N$ とします。そして復号については逆に $e^{-1}$ 乗します。具体的にはカーマイケルの定理から $(\mathbb{Z}/pq\mathbb{Z})^×≅\mathbb{Z}/\mathrm{lcm}(p−1, q-1)\mathbb{Z}$ より $\phi(N) = (p - 1)(q - 1)$ として $d = e^{-1} \pmod{\phi(N)}$ 乗すれば $m$ を復号できます。
...
ここで $N, e$ を知っていても $p, q, d$ いずれも知らないとき、素因数分解の計算困難性を仮定すると復号は難しいと証明されています。これを用いた暗号をRSA暗号 (Rivest-Shamir-Adleman encryption) と呼びます。
...
4. Alice は秘密鍵 $p, q$ を用いて復号し、平文を得る
...
それとわかりやすいように暗号化, 復号関数 $\mathcal{E}_{pk}, \mathcal{D}_{sk}$ を定義しておきます。
