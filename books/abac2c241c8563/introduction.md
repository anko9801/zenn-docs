...
### 対象読者
これは辞書的なものなのでこれを参考にするといいかもしれません。
- 暗号自体を軽く調べたい方
...
は自分と相手以外の人に内容が伝わらないように,かつ改竄されないように文章を送るための技術・理論の総称であり,多くの関連分野を含む
...
まずユーザーは信頼された第三者機関である認証局 (CA) に許可を得て、所有者情報や有効期限が書かれた電子証明書を発行してもらいます。そしてユーザーは公開鍵暗号の秘密鍵を用いて署名し、公開鍵と署名をホストに渡します。ホストは署名を公開鍵で復号し、CA で検証してもらってユーザーの正当性を保証します。逆に攻撃者がその人になりすます為には証明書を後から署名しなければなりませんが、ホストに保持された公開鍵に対応する秘密鍵を持っていないので署名は不可能です。
...
こうすることでデータがランダムに書き換わると送られてきたハッシュ値とは異なるハッシュ値となり、改ざんを検知できるという訳です。ま、これだけだと以前に送った暗号文に入れ替えるリプレイ攻撃に弱いですが。
