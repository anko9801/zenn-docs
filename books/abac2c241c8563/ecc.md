...
どうしてこんな形になるかというと右辺 $x^3 + ax + b$ は $x$ の 3 次関数なので 2 回ぐにょんぐにょんします。

![](/images/cubical.png)

そしてこの $x$ 軸に鏡を置いたらちょうど楕円曲線みたいな形になりませんか？これは $y^2 = k$ を解くと $y = \pm\sqrt{k}$ となるので $k \geq 0$ の値をプラスマイナスに置くのと同じです。

そしてこの楕円曲線の点同士について足し算ができます。といっても普通にベクトルとして足したら楕円曲線上に乗らないので、絶対に乗るような方法で足し合わせます。
