...
## 楕円曲線
半径 1 の円は次のような式で表されました。

$$
x^2 + y^2 = 1
$$

楕円曲線はこの 3 次方程式バージョンです。
...
ここからは特に素数 $p$ を用いて $q = p^n$ とおき、有限体 $\mathbb{F}_q\cong \mathbb{F}_p[x]/(f(x))$ のときについて考える。
...
位数の範囲が分かったので次は楕円曲線が与えられたときに実際に位数を計算する方法を考えます。これについてはフロベニウス写像の特性多項式を理解する必要があるのですがこの導出は微分とかいろいろ前提知識が必要なので飛ばします。The Arithmetic of Elliptic Curve に載ってるので興味ある人は読んでください。
