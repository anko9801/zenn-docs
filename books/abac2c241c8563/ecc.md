...
楕円曲線の理論は群環体、ガロア理論、可換環論、代数幾何学と理解した先になるので付け焼き刃程度しか書けないです。こういう知識は基礎を学ぶことで初めて応用が効くようになりますが、ちょっと触る程度で終わってしまうと何も応用が効かない狭い知識に成り果ててしまいます。そうはなりたくなければ、1つ1つ自分で理論を構築できるようになるまで理解すると良いと思います。

## 楕円曲線
TODO: かなり書き直す

> **Def. 楕円曲線**
> $K$ を体、 $f(x) \in K[x]$ を**相異なる 3 解をもつ** 3 次方程式としたときに関数 $y^2 = f(x)$ を楕円曲線という。また $x, y$ の線形変換によって 2 次項を消すことができ、次のように書ける。
>
> $$
y^2 = x^3 + ax + b \qquad ()
$$

相違なる 3 解をもつというのは特異点がない曲線であることと同値である。
