...
体の標数 $\ch(K)$ とは $1$ を何回足したら $0$ になるのかという数です。これよりどんな数も標数倍すると必ず 0 となり、ゼロ除算はできないので標数で割ることは出来ません。例えば素体 $\mathbb{F}_p$ は標数 $p$、有理数体 $\mathbb{Q}$ や実数体 $\mathbb{R}$ は標数 $0$ です。代数閉包 $\overline{K}$ は $K$ 係数多項式の解を取り込むことを変わらなくなるまで繰り返した代数構造です。例えば実数体 $\RR$ の代数閉包は代数学の基本定理より複素数体 $\CC$ です。
