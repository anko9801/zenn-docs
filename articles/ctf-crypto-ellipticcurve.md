...
標数とは $1$ を何回足したら $0$ になるのかという数です。これよりどんな数も標数倍すると必ず 0 となり、ゼロ除算はできないので標数倍で割ることは出来ません。例えば素体 $\mathbb{F}_p$ は標数 $p$、有理数体 $\mathbb{Q}$ や実数体 $\mathbb{R}$ は標数 $0$ です。

以下では特に断りがない限り標数が 2, 3 ではない有限体の楕円曲線を扱います。
