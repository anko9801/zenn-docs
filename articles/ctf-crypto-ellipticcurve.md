...
体とは四則演算ができる代数構造です。実数 $\mathbb{R}$ や有理数 $\mathbb{Q}$ などは体ですし、素体と言われる $\mathbb{F}_p$ は素数 $p$ で割った余りでこれも体となります。他にも素体の多項式上の演算である拡大体 $\mathbb{F}_{p^n}$ などもあります。

標数とは $1$ を何回足したら $0$ になるのかという数です。これよりどんな数も標数倍すると必ず 0 となり、ゼロ除算はできないので標数倍で割ることは出来ません。以下では特に断りがない限り標数は 2, 3 ではないとします。そして特に標数 0 である実数体 $\mathbb{R}$ 上の楕円曲線は次のようなグラフとなります。
