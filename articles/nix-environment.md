...
今 LLM が環境整備しようとすると、スクショを撮って、判断して、クリックするの繰り返しです。スクショという重い処理を何十回も行うし、並列化しにくいフローで手作業で 5 分掛かるところが 20 分くらい掛かります。このボトルネックは LLM モデルの問題ではなく、LLM の強みを全く活かしきれてない OS を含めた設計の問題で、そう簡単には解消されそうにありません。
