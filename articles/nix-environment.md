...
まず考えておくべきは環境整備を LLM が GUI 操作して行うのはどうでしょう。

現状はスクショを撮って、考えて、クリックするの繰り返しをしています。スクショという重い処理を何十回も行うし、並列化しにくいフローで手作業でやった方が断然早い状況です。このボトルネックは LLM モデルの問題ではなく OS を含めた設計の問題で、そう簡単には解消されそうにありません。

この根本となるのは環境がテキストで書かれていないことです。LLM が得意なのはテキストの読み書きであるのにそれを活かせてないというが原因です。

Nix は宣言的に環境を記述できる為に他のツールよりも把握がしやすく LLM との相性がいいです。

これを推し進めて読み書きするのを完全に LLM に任せることができればインターフェースになります。画面をぽちぽちせず、何か設定したいことを話すだけで環境が整えられるようになります。
