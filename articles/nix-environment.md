...
そもそも dotfiles という言葉は Unix 開発者が意図せず生み出した設計上の副産物に起因するものです。
...
それぞれメリデメありますが、怠惰な僕にとっては yadm が好みです。git と全く同じインターフェースで覚えることが少ないし、直接管理するから Single Source of Truth でわかりやすいです。
...
Nix はビルドシステムかつパッケージマネージャーです。さまざまな機能があり、すべての基盤は Nix 言語で環境を宣言的に記述し、Flakes という仕組みで依存するパッケージのバージョンを `flake.lock` に固定して再現性を保つ仕組みです。
