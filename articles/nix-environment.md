...
Nix で環境をテキストで記述すると、このボトルネックが解消します。しかもインターフェースが「画面をぽちぽちする」から「話すだけ」になります。

使い方は単純です。欲しい機能があれば聞く。
...
LLM は Nix の現状のファイル群を読んで、既存の設定と競合しないか確認しながら提案を返してきます。採用するなら `nix run .#switch` を叩くだけで環境に反映されます。

ここで効いてくるのが philosophy ファイルです。`docs/` 以下に自分のツール選定基準を書いておくと、LLM の提案がその基準に沿ったものになります。たとえば「シェルの起動は 200ms 以内」「POSIX 互換であること」と書いておくと、Fish や Nushell は最初から候補に上がらなくなる。「自分の好み」を毎回説明しなくていい。

哲学を書くほど提案の精度が上がる。採用するたびに環境が育つ。さらに気になった記事や他人の dotfiles を渡せば、LLM が自分の哲学と照らし合わせて比較検討してくれます。これを大量の記事に対してやると、一人では到底読みきれない量の知恵から自分の理想の環境を突き詰めていける。
