...
LLM によってコードを書いたり、設計を考えたり、調べものをしたりと、あらゆる作業が速くなりました。なのになぜ、作業環境の管理は相変わらず自分でやっているのでしょうか。

先日、複数あるメールクライアントの一本化に LLM を使って GUI 操作を任せてみました。すると、スクリーンショットを撮って判断してクリックの繰り返しという並列化しにくいフローになっており、自分でやれば 5 分程度で終わるのに 20 分程度掛かりました。このボトルネックは LLM モデルの問題ではなく OS を含めた設計の問題で、そう簡単には解消されそうにありません。

こうした手間は、PC を買い替えるたびに、転職するたびに、サーバーを立てるたびに、何度もやり直すことになります。その上、開発効率が上がり世の中にアプリが増えてきて管理しきれず、日々新しいアプリが出ているけれども、多すぎて一番良いのを見つけるのがむずかしかったり、すっと手が伸びにくくなってるんじゃないでしょうか。

そこで今回は、エンジニアが作業環境を管理するために使ってきた dotfiles に環境を宣言的に記述し、LLM のコンテキスト理解とのシナジーで効率良く管理する方法を紹介します。

## dotfiles はどう進化してきたか

私自身、dotfiles を長いこと管理してきて、シェルスクリプトで管理し始め、chezmoi を試し、Ansible に冪等性の良さを教えてもらい、yadm に移行し、 Nix に落ち着きました。dotfiles の管理方法はこれだけ選択肢があって、どれも一長一短あります。

### dotfiles の起源

そもそも dotfiles という言葉は Unix 開発者が意図せず生み出した副産物によるものです。

ls コマンドの開発中、カレントディレクトリ `.` と親ディレクトリ `..` が表示されるのが煩わしいため、先頭がドットのファイルをすべて非表示にする処理が加えられました[^1]。それが意図せずして隠しファイルという仕組みが生まれ、あらゆるアプリが設定ファイルをドットから始めるようになりました。それらをまとめて dotfiles と呼びます。

### 設定を同期したい

PC はエンジニアにとって毎日朝から晩まで使う道具です。少しでも快適にしようと、便利なツールを入れ、設定を調整し、キーバインドを覚え直す。そうやって積み重ねた設定ファイルが、ホームディレクトリにドットから始まるファイルとして溜まっていきます。

そうして育ってくると問題は複数のマシン間で設定を同期するかになります。これがないと私用 PC で使っていた `.zshrc` が社用 PC で使えなかったり WSL を入れ直したら最初からで不便なままです。

そこでリポジトリに dotfiles を入れて管理しています。これで同期できるし、他人の知恵や工夫も参考にできます。
...
Nix はビルドシステムかつパッケージマネージャーです。Nix 言語で環境を宣言的に記述し、Flakes という仕組みで依存するパッケージのバージョンを `flake.lock` に固定します。
...
これらを使ってすべてのツールを Nix で宣言的に管理すると、dotfiles は単なる設定集を超えてマシン環境そのものの記述になります。つまり環境のスナップショットがとれて、壊れたり紛失したり水没して新しいマシンを買っても、いつでも以前の状態に戻せるのでバックアップがほとんど不要になります。また具体的な操作から離れてどんな環境にしたいかだけを考えればよくなります。ただしデメリットもあります。
...
4. 動的型付けで LLM との相性が完全ではない
  Nix 言語は動的型付けのため、LLM が提案したコードが正しいかどうかの検証が実行時まで分かりません。「読ませる」には強いですが、「書かせる」精度は静的型付け言語には劣ります

### Nix × LLM のシナジー

LLM に GUI 操作をさせるのはスクリーンショットという重い操作を並列化できないところにあります。

LLM が本来得意なものは、大量のテキストを読み書きすることなのに対極のことをしています。

環境がどういう状態かを読める形で書かれている必要があります。既存の手法で試すとこうなります。
...
Nix は便利ですが、dotfiles を育て続けるのは手間がかかります。よさそうな設定を見つけ、自分の環境にどう入れるかを考え、Nix に翻訳して書き、適用して動作確認する。壊れたら直して、他のツールとも比較検討して、全マシンに反映する。特に試行錯誤の段階が重く、ツール一本の切り替えに 10 分かかることもあります。
ここで LLM が役に立ちますが、LLM が得意なのは操作ではなく差分の提案です。「今の状態」と「理想の状態」を比べて、何を変えるべきかを 1 度の呼び出しで返します。そのためには環境が「今どういう状態か」を読める形で書かれている必要があります。
既存の手法では限界があります。

Ansible / shell script — 命令型。「過去にやったこと」の手順書であって、今の状態ではありません
Homebrew のリスト — 何が入っているかはわかりますが、なぜ入っているか、何に依存しているかが暗黙的です
chezmoi / yadm — 設定ファイルの同期はできますが、依存関係が見えません

Nix の flake.nix を渡すと、現在の環境が依存関係ごとすべて一箇所に宣言的に書かれています。LLM にとって環境全体を俯瞰できる地図が出現します。するとサイクルがこう変わります。

LLM で大量の記事をリサーチさせて改善点を洗い出す
自分の判断基準をもとに採否を決めて適用する
壊れてもロールバックで即座に戻せる
全マシンに適用する

さらに「どんな環境にしたいか」という自分の哲学を文章にして渡せば、LLM の提案が意図に近づき、採否の判断コストも下がっていきます。週次で改善提案を通知する仕組みを組んでおけば、あとは流れてくる提案に採用・却下を返すだけで環境が育ち続けます。
...
LLM に操作をさせると遅い理由は単純で、操作はシリアルな処理だからです。スクリーンショットを撮って判断してクリックして、また撮って判断して……という繰り返しが何十回も積み重なります。LLM 呼び出しが 1 回数秒でも、50 回で数分になります。
一方で LLM が本来得意なのは、大量のテキストを読んで差分を考えることです。「今の状態」と「理想の状態」を比べて、何を変えるべきかを 1 度の呼び出しで提案します。
この得意なことを発揮するには、環境が「今どういう状態か」を読める形で書かれている必要があります。既存の手法で試すとこうなります。

Ansible / shell script — 命令型。「過去にやったこと」の手順書であって、今の状態ではありません
Homebrew のリスト — 何が入っているかはわかりますが、なぜ入っているか、何に依存しているかが暗黙的です
chezmoi / yadm — 設定ファイルの同期はできますが、依存関係が見えません

どれも動かないわけではありません。ただ LLM に渡したとき、提案の質と一気通貫性が全然違います。
Nix の flake.nix を渡すと、現在の環境が依存関係ごと、すべて一箇所に宣言的に書かれています。「このツールが入っている」だけでなく「このバージョンで、この設定で、この依存関係で入っている」が全部見えます。LLM にとって環境全体を俯瞰できる地図が出現します。
しかも提案を受け取った後、nix switch の 1 コマンドで環境ごと適用できます。読む・提案する・適用するが一気通貫で回ります。これが Nix と LLM の相性のよさの正体です。
...
このような設計を一瞬で始められるようにテンプレートを用意しました。
...
こんな考えで作った dotfiles です。こんな感じでどこでも適用して 15 分くらい待つだけで積み上げてきた自分の最高の環境が手に入るという訳です。
