...
## Nix × LLM で環境整備のインターフェースが変わる

LLM が環境整備しようとすると今はこんな流れになります。スクショを撮って、判断して、クリックする、この繰り返しです。スクショという重い処理を何十回も行うし、並列化しにくいフローなので手作業で 5 分のところが 20 分かかったりします。これはモデルの問題ではなく、LLM の強みをまったく活かせていない設計の問題です。

Nix で環境をテキストで記述すると、このボトルネックが解消します。

しかもインターフェースが「画面をぽちぽちする」から「話すだけ」になります。欲しい機能があれば聞いて

```
ターミナルで作業ディレクトリを素早く切り替えたい
```

知らないツールが出てきたら調べてもらう

```
zoxide というツールを見かけた。自分の環境に合うか調べて、
合うなら追加して
```

こんな感じで LLM は Nix の現状のファイル群を読んで、既存の設定と競合しないか確認しながら提案を返してきます。採用するなら `nix run .#switch` を叩くだけで環境に反映されます。

ここで効いてくるのが philosophy ファイルです。`docs/` 以下に自分のツール選定基準を書いておくと、LLM の提案がその基準に沿ったものになります。たとえば「シェルの起動は 200ms 以内」「POSIX 互換であること」と書いておくと、Fish や Nushell は最初から候補に上がらなくなる。「自分の好み」を毎回説明しなくていい。触っている内に新しい基準に気付いて追加するようになる。

このループが回り始めると、環境整備を「やる」という感覚がなくなっていきます。
...
設定が増えてくると、ツール間の依存関係が複雑になり LLM でも全体を把握しきれなくなり、精度が下がります。これを防ぐために、責務を切り分けて管理しやすい構造にして `AGENTS.md` をハブにします。
