...
ls コマンドの開発中、カレントディレクトリ `.` と親ディレクトリ `..` が表示されるのが煩わしいため、先頭がドットのファイルをすべて非表示にする処理が加えられました。それが意図せずして隠しファイルという仕組みが生まれ、あらゆるアプリが設定ファイルをドットから始めるようになりました。それらをまとめて dotfiles と呼びます。なお Rob Pike は 2012 年にこれを振り返り、間違いだったと述べています[^1]。
