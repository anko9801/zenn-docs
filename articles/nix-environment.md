...
LLM が得意とするのは、差分を見ることです。

「今」と「あるべき姿」のあいだに橋を架け、何を変えるべきかを一度の呼び出しで返す。そのために必要なのは、今の環境が*読める形で書かれている*こと。手続き的な呪文の羅列ではなく、状態そのものが文章として存在していること。

Nix は、ちょうどそれだ。

宣言的な設定ファイルは LLM にとって理想的な入力だ。「このマシンには何が入っていて、なぜ入っているか」が一枚の地図として広がっている。だから提案の質が変わる。文脈を読んで、一気通貫で、答えてくれる。

そうすると何が起きるか。

LLM が環境の総合的なインターフェースになる。アプリを入れてほしければ話しかければいい。改善してと言えば、入っていない流行りのツールを調べて候補を出してくれる。なぜこれが入っているのかを聞けば、過去のコンテキストを辿って答えてくれる。ツールごとに異なる UI を覚える必要はない。設定ファイルの書き方を一から学ぶ必要もない。操作は全部、自然言語だ。

さらに「どんな環境にしたいか」という自分の哲学を文章にして渡せば、提案が意図に近づく。採否の判断コストが下がる。週次で改善提案を受け取り、「採用」か「却下」を返すだけで、環境が育ち続ける。

失敗しても、怖くない。ロールバックが一瞬だから。

これは、効率化ではない。

人間と開発環境の関係が、変わる。

ただし、そのままだと精度が悪いです。まず書く方は Nix の型システムはまだ成熟しておらず、LLM が生成したコードの意味的な正しさを保証できません。linter で緩和。読む方は設定が増え、依存が絡み合えば、LLM は文脈を失う。次のセクションの設計で解決する。
