...
LLM を使ってコードを書いたり、設計したり、調べものしたりと、あらゆる作業が速くできるようになりました。なのになぜ環境整備は相変わらず自分でやっているんでしょうか？

先日、複数あるメールクライアントの一本化に LLM を使って GUI 操作を任せてみました。すると自分でやれば 5 分程度で終わるのに 20 分程度掛かりました。スクリーンショットを撮って判断してクリックの繰り返しという並列化しにくいフローになっており、このボトルネックは LLM モデルの問題ではなく OS を含めた設計の問題で、そう簡単には解消されそうにありません。

こうした手間は、PC を買い替えるたびに、転職するたびに、サーバーを立てるたびに、何度もやり直すことになります。その上、開発効率が上がり世の中にアプリが増え、改善記事も増えて管理しきれず、日々新しいアプリが出ているけれども、一番良いのを見つけるのがむずかしくてすっと手が伸びにくくなってるんじゃないでしょうか。
...
まずはその歴史を見ながらそれぞれのツールを体系的に解説します。
...
根幹となるのは環境を宣言的に記述する Nix 言語と依存パッケージのバージョンを固定をする Flakes という仕組みです。たとえば Flakes のエントリーポイントとなる `flake.nix` はこんな感じです。これをもとにバージョンが固定された `flake.lock` が生成されます。
...
「今の状態」と「こうしたい」をどちらも文章で渡せば、そのあいだを埋めてくれます。そのために必要なのはひとつ——今の環境が、*読める形で書かれている*こと。手続き的な呪文の羅列ではなく、状態そのものが文章として存在していること。

Nix は、ちょうどそれです。

宣言的な設定ファイルは、LLM にとって理想的な入力になります。「このマシンには何が入っていて、なぜ入っているか」が一枚の地図として広がっています。だから提案の質が変わります。文脈を読んで、一気通貫で、答えてくれます。

そうすると、何が起きるでしょうか。

LLM が環境の統一インターフェースになります。アプリを入れてほしければ、話しかければいい。改善してと言えば、まだ手元にない流行りのツールを調べて候補を出してくれます。なぜこれが入っているのかを聞けば、設定の来歴を辿って答えてくれます。ツールごとに異なる UI を覚える必要はありません。設定ファイルの書き方を一から学ぶ必要もありません。操作はすべて、自然言語です。

さらに「どんな環境にしたいか」という自分の哲学を文章にして渡せば、提案が意図に近づきます。採否の判断コストが下がります。週次で改善提案を受け取り、「採用」か「却下」を返すだけで、環境が育ち続けます。

失敗しても、怖くありません。ロールバックは一瞬だから。
...
*書く*精度については、Nix の型システムはまだ成熟途上にあり、LLM が生成したコードの意味的な正しさを自動では保証できません。これは linter による静的検査で緩和します。

*読む*精度については、設定が増え依存が絡み合うにつれ、LLM はしだいに文脈を失います。これは次のセクションで述べる設計によって解決します。
...
HIGH Priority
  ...
  │ カテゴリ │                       内容                       │             補足             │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Claude   │ env: CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR,   │                              │
  │ Code     │ ENABLE_BACKGROUND_TASKS, ENABLE_TOOL_SEARCH,     │ セッション安定化             │
  │          │ DISABLE_AUTOUPDATER                              │                              │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Claude   │ /compress コマンド（ドキュメント最適化）         │ CLAUDE.md 等の圧縮用         │
  │ Code     │                                                  │                              │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Claude   │ 公式プラグイン有効化（code-review,               │                              │
  │ Code     │ security-guidance, code-simplifier）             │                              │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Git      │ lockb diff driver 定義（textconv = bun; binary = │ 属性はあるがドライバー未定義 │
  │          │  true）                                          │                              │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Git      │ cd alias（デフォルトブランチに switch）          │                              │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Ghostty  │ quick-terminal-animation-duration = 0            │ 即表示                       │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Ghostty  │ macos-window-shadow = false                      │ AeroSpace タイリング最適化   │
  ├──────────┼──────────────────────────────────────────────────┼──────────────────────────────┤
  │ Ghostty  │ macos-non-native-fullscreen = visible-menu       │ SketchyBar 互換              │
  └──────────┴──────────────────────────────────────────────────┴──────────────────────────────┘
  ...
  │  カテゴリ  │                  内容                  │                 補足                 │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Zsh        │ HIST_ALLOW_CLOBBER                     │ CLOBBER off 時の history recall 修正 │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Zsh        │ HIST_NO_FUNCTIONS                      │ history から関数定義除外             │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Zsh        │ nocorrect aliases（mkdir, mv, cp, rm,  │ 誤correction防止                     │
  │            │ sudo）                                 │                                      │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ macOS      │ EnableTiledWindowMargins = false       │ ネイティブタイリング gap 削除        │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ macOS      │ Finder NewWindowTarget = "PfHm"        │ Recents→Home                         │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Git        │ *.local を wt.copy に追加              │                                      │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Mise       │ pin = true, lockfile = true            │ 再現性向上                           │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Ghostty    │ bold-is-bright = true                  │ bold を bright カラーに              │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Ghostty    │ quick-terminal-screen = mouse          │ マルチモニタ対応                     │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Ghostty    │ macos-titlebar-style = tabs            │ タブ統合でスペース節約               │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ AI/MCP     │ context7, gh_grep MCP サーバー         │ ライブラリドキュメント/GitHub        │
  │            │                                        │ コード検索                           │
  ├────────────┼────────────────────────────────────────┼──────────────────────────────────────┤
  │ Claude     │ 新 permissions フォーマット移行        │ allow/deny 配列形式                  │
  │ Code       │                                        │                                      │
  └────────────┴────────────────────────────────────────┴──────────────────────────────────────┘
  ...
  │ CLI      │ ffmpeg パッケージ追加                       │
  ├──────────┼─────────────────────────────────────────────┤
  │ Mise     │ activate_aggressive, install_before = "12h" │
  ├──────────┼─────────────────────────────────────────────┤
  │ macOS    │ fnState = true（F キー標準化）              │
  ├──────────┼─────────────────────────────────────────────┤
  │ Zsh      │ PUSHD_TO_HOME                               │
  ├──────────┼─────────────────────────────────────────────┤
  │ Browser  │ Finicky（URL ベースブラウザルーティング）   │
  ├──────────┼─────────────────────────────────────────────┤
  │ Git      │ merge.ff = false                            │
  └──────────┴─────────────────────────────────────────────┘
```
