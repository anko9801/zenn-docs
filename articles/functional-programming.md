...
「ソフトウェアエンジニアリングとは時間で積分したプログラミングである」ということ

短時間しか扱わないコードは書き捨てていい。しかし業務で書くコードは開発・修正・保守を回す必要があり、スケーラブルでなく、1タスクに掛かる時間がコードの規模に応じて増えていきます。コードの規模が倍に増えたら労働時間も倍になる。

その中でクリティカルに効率が変わるものは次の 3 つがあります。

1. わかりやすい
2. デバッグが楽ちん
3. 変更に強い

これらをどのように言語機能で実現するのか、まずはこれらの原理を理解しましょう。

### わかりやすさ = 脳の負荷を下げること

コードは書く回数より読む回数の方が多いです。新しい機能を追加するときに読みます。その為、書きやすいコードより読みやすいコードを優先した方が 10 倍効果があります。

まず 2 つの文を読んでください。

---

A: 本を私は買った。昨日、プログラミングの本屋で、駅の近くにある。

B: 私は昨日、駅の近くにある本屋で、プログラミングの本を買った。

---

文 B の方が読みやすい。理由は語順で、関係のある単語が近くにあると、頭の負担が減ります。文 A では「駅の」と「近くにある」、「プログラミングの」と「本」が離れていて、文 B ではこれらが隣にあります。文 A では意味を繋ぎ合わせるために、頭の中で単語を保持しながら読まなければなりません。

この「脳の負担」を「認知負荷」といって、頭の中のメモリである「ワーキングメモリ」を消費します。ワーキングメモリの容量は 5 〜 9 個の情報までで時間は約 20 秒しか持たず、そのキャパシティを越えると頭がパンクします。つまり **わかりやすいとはワーキングメモリを節約する** ことです。実は数学が難しく感じるのは論理が難しいからだけではなくて、いろんな可能性を考えてる内に頭がパンクしちゃうからが主なのです。
...
これを次のようにしたらわかりやすいでしょう。

```
```

よりわかりやすいコードを書く為に他にもたくさんの方法があります。
...
| 深いネスト | 浅いネスト |
| 命令的 | 宣言的 |
| 大きなひとかたまりのコード | 意味で分離する |
| 関係ないものが混在 | コロケーションする |
| 実装が名前やコメントより軽い | 名前を省いて直接書く |
| 重複が多い | Single Source of Truth, 階層構造 |
| デッドコード | 消す |
| スパゲッティフロー | 単方向フロー |
| 多数の依存 | 少数の依存 |
| 知らない手法 | 普段よく使う手慣れた方法 |

例えばフロントエンドでは 1 つの状態が複数の UI に影響し、 1 つの UI 操作が複数の状態を変更します。このような複雑な循環構造を View ⇄ Model のようにそのまま相互に依存するように書いてしまうと、大規模になったとき依存関係が爆発して、バグった時に疑わしい場所が数百個も出てきて、どこのフローから出現してどれが原因かわからなくなってしまいます。これに対して View → Update → Model → View のような単方向フローにすることで依存関係を減らすことができます。これが The Elm Architecture や Redux などの状態管理アーキテクチャです。

Simple Made Easy という言葉がありますが、少人数であれば主観的に手慣れている Easy でも機能しますが、関わる人数が増えるほど誰にとってもワーキングメモリが少ない Simple がいいです。

このワーキングメモリ 1 つ覚えておけばソフトウェアエンジニアリングのほぼ全てのことを説明できます。プログラミング以外でも UI/UX、発表、コミュニケーション、組織構造、整理整頓、接客などなど非常に広範に適用できる考え方です。人間はどれくらいワーキングメモリを使ってるのか感じ取るのは不得意なので落ち着いて情報がいくつあるのかを数えると、仕事効率が飛躍的に上がります。

### デバッグが楽ちん = 合成性・代数的

人はよく間違いを犯します。
ソフトウェア工学ではたくさんのルールがありますが、このワーキングメモリ 1 つ覚えておけばほぼ全てのことを説明できるのです。プログラミング以外でも UI/UX、発表、コミュニケーション、整理整頓、接客などなど非常に広範に適用できる考え方です。
自然の物理に近い方がデザインの認知負荷が低い
指で弾くと ease-out

その間違いが指摘されずに数千行書いたあとで間違いが発覚すると修正が大変です。逆に間違いを瞬時に指摘してくれると最もコスト低く正しいコードが書けます。現代はリンターやその他の静的コード解析ツールを使ったりしてエラーや警告を出してくれたり、CI を用いて正しい挙動となっているかを確かめたりしてくれてその環境は整っているように見えます。

しかし、大規模なシステムになってくるとすべてを指摘できなくなり、注意していないとデバッグが非常に困難になります。じゃあどうしたらいいかというと、間違いを起こさないような設計をすればいいんです。

その方法は単純なものを組み合わせて複雑なシステムを作ることです。複合した意味が構成要素の意味のみで決定されるという性質を合成性 (Compositionality) と言って、例えばタイプで分けて
...
# スレッドA: x を読んで x+1 を書く
# スレッドB: x を読んで x*2 を書く
```

局所的な部分をみて性質が担保されること。

バグを見つけて直すとき次の手順を踏み、それぞれ次のような恩恵を受けられます。
...
### 変更に強い = 変わりにくいものに依存する・依存を減らす

プロジェクトを進めると、仕様が変わる。データベースが PostgreSQL から MongoDB になる。UI が Web からモバイルアプリになる。決済方法が増える。

1 つの変更が次に波及すると、それに伴う変更が多数の変更を引き起こし、さらに変更が多数に伝播して雪崩のように伝播していく。サービス A がサービス B の API を使っていて、サービス A を変えるとそのサービス B も変えなければならない。こういったシステムは避けようと思っても往々にしてあると思います。

仕様変更に強くするには、**変わりやすいものが、変わりにくいものに依存する**ことです。これにより変更の波及を最小化します。これを実践したのがクリーンアーキテクチャで「ドメイン ← ビジネスロジック ← 技術的詳細」と安定性と依存性の方向が一致するように組み立てています。[^clean]

もしくはそもそも依存しないようにすること。分離する
...
これらを満たすようなプログラミング言語を作れれば良さそうです。それが関数型言語なのです。今回は純粋関数、モナド、圏論と順を追って実用性を追求してみます。
...
開発している中で純粋関数は利点がありません。では、いつ役立つのかというとテスト・デバッグのタイミングです。そう、純粋関数は合成的であることの大きな具体例なのです。

まず何も考えず副作用とロジックが混在したコードのデバッグを考えます。

バグを再現するとき、全ての状態を再現する必要があります。
...
合成的である恩恵に独立に計算できるので並行処理・最適化がしやすい。

さらに分離ができて

そうは言ってもパフォーマンスのために変数を使ったり、I/O やネットワークなどはよく使います。そのために副作用は必ず影響範囲がどこかマークして限定し、その範囲外のコードは純粋に保つことができれば認知負荷の高い部分を最小限に抑えられます。Haskell は普段書いているプログラムはすべて純粋にしてその外側で副作用を隠蔽しています。どのように扱うかというと次の章のモナドを用いて表します。
...
これらの射の表現力をあり得る関数の数 (型の濃度) で測ると次の表になります。
...
モナドでは解決不可能な問題でした。そこでモナドの上位互換として Algebraic Effects & handlers がでてきました。
...
例外と聞くと嫌悪感を抱く人が少なからずいるかもしれませんが、ご安心ください。例外の悪いところを整理すると catch したときに何の例外が発生したのかを型で明確にできるので制御フローをより明確にすることが可能になります。Effect の合成がモナドよりも簡潔になります。
