...
標準入出力と乱数を使ったプログラムを書く時点でモナドは破綻します。なのでモナドを合成する手段が必要です。それがモナド変換子です。`ReaderT IO a` のように積み重ねていきますが、
...
Effect をユーザー定義することができ、より柔軟な表現力を持ちます。Effect と Handler を分離し、ある Effect を呼び出すと対応する Handler が見つかるまで上位の処理に遡り、実行し、その後の処理を再開したり、停止したり、後で実行したりできます。これにより、同じコードでもテスト時はモック、本番では実際の DB といった切り替えが容易になります。
