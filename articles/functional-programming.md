...
専門用語が多すぎるし、説明が長い！使う理由がよくわからない！数学みたいにシンプルで必要なことだけ書いて欲しい！

とそんな人が最近本腰を入れて関数型に向き合ってみた記録になります。実用性を
...
この「頭の負担」を**認知負荷**といいます。認知負荷の正体は**ワーキングメモリの消費**です。ワーキングメモリとは頭の中のメモリのこと。 容量は 5 〜 9 個の情報までで時間は約 20 秒しか持たない。キャパシティを越えると認知限界。わかりやすいとは、 **ワーキングメモリを節約する**ことです。実は難しいと感じるのはロジックが悪いのではなくメモリを多く消費するからなのです。

プログラミングでも同じことが言えます。以下のような工夫をすることで、よりわかりやすいコードを書くことができます。
...
| 命令的 | 宣言的 |
| 依存関係の複雑さ | 合成的 |
| 長い再現しないコード | 最小の再現コード[^minimal] |

Simple Made Easy という言葉がありますが、Simple は誰にとっても少ないワーキングメモリであり、Easy は主観的に手慣れたものです。例えば for while if 文が 1 つの統一的な方法で記述できたとして easy を取った方がいいです。

階層構造は Single Source of Truth と単方向フローの組み合わせですし、コロケーションも

特にフロントエンドでは 1 つの状態が複数の UI に影響し、 1 つの UI 操作が複数の状態を変更する。このような複雑な循環構造をそのまま相互に依存するように書いてしまうと大規模になったときスパゲッティフローで依存関係が爆発して破綻してしまう。View ⇄ Model ではなく View → Update → Model → View のような単方向フローにすることで依存関係を減らすことができます。これが The Elm Architecture や Redux などの状態管理アーキテクチャです。さらには Update のみをテストできるようになります。

ソフトウェア工学ではたくさんのルールがありますが、このワーキングメモリ 1 つ覚えておけばほぼ全てのことを説明できるのです。プログラミング以外でも UI/UX、発表、コミュニケーション、整理整頓、接客などなど非常に広範に適用できる考え方です。

このようなわかりにくいという問題は大規模になるほど他の問題に比べて圧倒的に大きくなります。なので落ち着いて情報がいくつあるのかを数えると、仕事の効率が大きく上がります。
...
それは**変わりやすいものが、変わりにくいものに依存する**ことです。変更の波及を最小化します。これを実践したのがクリーンアーキテクチャで「ドメイン ← ビジネスロジック ← 技術的詳細」と安定性と依存性が一致するように組み立てている。[^3]

仕様変更という変わりやすさだけではなく副作用という変わりやすさにも同様です。
...
プログラミングにおける最大の敵は「複雑さ」です。どこで変更されたかわからない状態、予測不能な挙動、これらがバグを生みます。
関数型プログラミングでは、この複雑さを「純粋関数」と「型」という理論的な道具で管理します。

### 純粋関数：副作用を閉じ込める

別に入出力あって分かりにくいって思ったことない。

関数型プログラミングを学び始めると最初に出会う重要な概念が純粋関数です。乱数生成や I/O、日付は純粋なのか？といった疑問が各地で議論されてます。その議論の前に整理することとして「純粋性」「副作用」「参照透過性」といった似た概念が混同されがちなので、 まずは定義をはっきりさせましょう。
...
> - 副作用がない : スコープ外の観測可能な状態に依存せず、影響も与えない
...
純粋関数と型で世界をきれいに保てることはわかりました。しかし、現実のアプリケーションは「副作用」の塊です。
データベースへの書き込み、API通信、ログ出力、エラーハンドリング。これらを純粋な世界でどう扱うか？
ここで登場するのが、圏論由来の**モナド**です。
...
圏論を知ってる人にはわかるんだろうよ。僕にはなにいってんのかさっぱりやねん。
...
| 型クラス        | 恒等射      | 射の型          | Maybe モナドにおける射        | 型の濃度     |
| ----------- | -------- | ------------ | --------------------- | -------- |
| Functor     | `id`     | `a -> b`     | なにも干渉できない             | $b^a$    |
| Applicative | `pure`   | `f (a -> b)` | `Nothing` という定数関数ができる | $f(b^a)$ |
| Monad       | `return` | `a -> m b`   | 引数によって `Nothing` にできる | $f(b)^a$ |
...
Haskell では `IO` モナドが副作用のマーカーとなります。`IO` 型の関数自体は純粋ですが内部実装で副作用を隠蔽しています。乱数や日付は `IO` モナドによって副作用があることがマークされます。

```haskell
readAndDouble :: IO Int
readAndDouble = do line <- getLine
  let num = read line
  return (num * 2)
...
既存のモナドに別のモナドの機能を追加（トランスフォーム）する形で作られます。`ReaderT IO a` のように積み重ねていきますが、スタックが深くなるとパフォーマンスが悪化したり、`lift` で持ち上げるのが大変だったり、$N$ 個のモナドに対して $N^2$ 通りの組み合わせ（インスタンス）を定義しなければならないという「$N^2$ 問題」があります。

```haskell
class (forall m. Monad m => Monad (t m)) => MonadTrans (t :: (Type -> Type) -> Type -> Type) where
  lift :: m a -> t m a
  -- 恒等律: lift . return = return
...
プログラムは「何をするか（Effect）」だけを記述し、ランタイムやハンドラが「どう実行するか」を決定します。これにより、同じコードでもテスト時はモック、本番では実際の DB といった切り替えが容易になります。
Koka や Unison、OCaml 5.0 などで採用されています。



### Hefty Algebras
Algebraic Effects の発展形で、**高階作用 (Higher-order Effects)** をモジュラーに扱うための理論です。
通常の Algebraic Effects は `get` や `put` のような単純な作用（一次作用）はうまく扱えますが、`catch` (例外捕捉) や `local` (Reader の局所変更) のように「計算そのもの」を引数に取る作用（高階作用）を扱う際、ハンドラの合成が難しくなる（モジュラーでなくなる）という課題がありました。
Hefty Algebras はこれを解決し、高階作用を含んだ Effect System をきれいに合成可能にするための代数的構造を提供します。これにより、複雑な作用を持つプログラムでも、各作用を独立して定義・実装・推論することが可能になります。

```haskell
-- 高階作用の例: Scope (計算の範囲を限定する)
data Scope m a where
  -- m a は「計算」を表す。計算を引数に取るのが高階作用の特徴
  Local :: (r -> r) -> m a -> Scope m a

-- Hefty Algebra ではこれを代数的にきれいに扱える
type HFunctor = (Type -> Type) -> Type -> Type
class HFunctor h => Hefty h where
  ...
```

## 圏論っていみあんの？

フレームワークは数年で廃れます。言語も流行り廃りがあります。
しかし、その背後にある「計算の仕組み」や「構造」は変わりません。
関数型プログラミングが強力なのは、その根底に**数学（圏論・型理論）**という普遍的な基盤があるからです。
...
型なしラムダ計算を集合論に落とし込もうとしたら、不動点を表す集合がなくて困りました。 $D\cong D\to D$ を満たす集合 $D$ がないといけないのですが有限集合なら濃度が $|D| < |D\to D|$ となるので難しそうです。
問題 $D\cong D\to D$ は1930 年代にあった

これを上手く集合に落とし込んだのが 1969 年の Dana Scott のドメイン理論です。これはボトム要素 $\bot$ をもつ完備半順序集合 (CPO) を考えると問題を解決できるぞって発見です。なんぞやと言われるとざっくりプログラムが停止しない時の値を持ちつつ極限を考えられて順序がある集合なんですが具体的には次の $D_\infty$ が $D\cong D\to D$ を満たす解の 1 つとなることがわかりました。
...
チャーチは $D \cong D\to D$ 問題で悩んでいた為、これを回避する為に、1940 年に単純型付きラムダ計算を発表しました。`λx: Int. x + 1` のように型という新しい概念を決めてあげて `λx. x x` のような型がマッチしないものは推論できないように制約を課したのです。表現力は落ちますが確実な方法です。
...
この型理論をより強化す
今まで内部の構造を作って見える性質を考えていたのですが外側から見た性質だけで話そうというのが圏論の魅力です。しかもこれまでの型付きラムダ計算をうまく説明することができるし、Eugenio Moggi によってモナドを使うことで Effect を扱えることを示しました。これらの発見を機に圏論を用いられるようになりました。
...
これまで「等しさ」を公理化することが
等しいという概念を定義せずとも
Cubical Type Theory は同値関係を Path で表し、連続写像を用いて証明を行います。


さまざまなモデル化が試されて淘汰されて残ったものが圏論。
...
共通する哲学は **「プログラムを『実行』するのではなく『記述（値）』する」** という点です。これを **代数的設計 (Algebraic Design)** と呼びます。「何をするか（記述）」と「どうするか（解釈）」を分離することで、プログラムは驚くほど柔軟になります。
...
### 2. 抽象化と作用 (Interpreter Pattern)

「データベースに保存する」「ログを吐く」といった副作用をどう記述するか。
...
値であれば、保存できるし、転送できるし、テストできるし、何より安全に組み合わせることができます。

関数型プログラミングを学ぶということは、無数にあるパターンを暗記することではありません。
...
[^3]: [世界一わかりやすいClean Architecture - 技術レイヤー分割より大切なモノ](https://www.docswell.com/s/nuits_jp/5DNXE9-easiest-clean-architecture)
