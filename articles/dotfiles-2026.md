...
基本的にカテゴリごとにディレクトリを分け、各 `.nix` ファイルが 1 つのアプリの責務を持っています。たとえば fish の設定を変えたかったら `shell/fish.nix` を編集します。ツールに依存する OS 差分は各ツールで条件分岐

通常ユーザーはすべて Home Manager を用いて設定し、NixOS macOS Windows の管理者は `system/` で管理しています。ツールに依存しない OS 差分は `system/` で吸収

ツール全体で共通しているカラースキーム、フォント、カーソルなどは `theme/` に書きます。ターミナルやエディターなどは全体的に一貫性をもった色合いになってくれます。

ツール同士の関係は変数で切り出して適切に配置

`config.nix` でホストごとユーザーごとに設定をわけることで環境の組み換えが楽になります。
