...
私はすべての機密情報を 1Password というパスワードマネージャーに置いています。他には Bitwarden Vaultwarden などがありますね。SSH や Git で秘密鍵が必要になったら Touch ID や Windows Hello などの生体認証で呼び出しています。API キーの環境変数は op コマンドで起動時に展開するようにして、漏洩防止として pre-commit に gitleaks を入れています。
