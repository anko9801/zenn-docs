...
しかしマシンは増える。私用 PC、社用 PC、コンテナ、クラウド。LLM で開発効率が上がった分だけアプリも加速的に増えて、それに比例して設定やキーバインドも増える。そんなの覚えてらんないし、それぞれのマシンにインストール方法しらべて前の設定みつけて入れるなんて苦行だよ～日々新しいアプリがでているけれど、すっと手が伸びにくくなってるんじゃないでしょうか。
...
あなたの dotfiles にも、まだ言語化されていない「なぜこのツールなのか」が眠っているはず。それを Nix で宣言し、`docs/` に書き出して LLM に読ませたとき、自分でも忘れていたこだわりが返ってくるかもしれない。
