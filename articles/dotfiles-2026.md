...
こだわりのキーボード、チェア、ガジェットやツールで日々生産性を上げていると思います。

しかし私用 PC、社用 PC、コンテナ、クラウドなどマシンが増え、LLM で開発効率が上がった分だけアプリも増えて、それに比例して設定やキーバインドも増える。そんなの覚えてらんないし、それぞれのマシンにインストール方法しらべて前の設定みつけて入れるなんて苦行だよ～と日々新しいアプリがでているけれど、すっと手が伸びにくくなってるんじゃないでしょうか。
