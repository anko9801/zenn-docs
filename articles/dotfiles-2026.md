...
さらにツールの選定基準を `docs/` で言語化して LLM に読ませることで、意図を汲み取った提案をしてくれるようになります。たとえば私であれば「認知負荷を徹底的に下げる」「キーバインドは目的語＋動詞で統一する」といった基準を書いています。不採用にした理由も残しておくことで、似たツールを再提案されなくなります。

こうした言語化を積み重ねることで LLM の提案精度が上がり、すべて任せて大丈夫な環境に育っていきます。
...
7. jujutsu (jj) の要否判断 — git と共存しているが使っているか？1 VCS に絞る方がシンプル
8. Fish の立ち位置の明確化 — zsh がプライマリなら fish は何のために？サーバー用？
```
