...
`malloc` 関数が呼ばれると malloc.c の次の関数を読めばわかります。
関数などを呼びながらメモリを確保する。
- `__libc_malloc` 関数
  - `malloc` 関数のエイリアスとなっている関数。主に tcache の処理を行う。
- `_int_malloc` 関数
  - malloc のほぼ全てが書かれてある。最も重要！
- `sysmalloc` 関数
  - その他 mmap など
- `malloc_consolidate` 関数
  - fastbins の統合を行う。


`__libc_malloc` 関数の擬似コード
```c
...
なぜこんな長い関数になってるかは inline 化して高速化したいのかと思います。
