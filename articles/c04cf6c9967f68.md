...
とりあえずこの伝説の動画を見て [**最新の malloc.c のソースコード**](https://elixir.bootlin.com/glibc/latest/source/malloc/malloc.c) を用意しましょう。
...
ヒープ領域ではチャンクと呼ばれる構造体が大量にあり、それぞれ使われているチャンクだったり、解放されたチャンクだったりします。`malloc` すると解放されたチャンクの中からチャンクが切り出され、そのデータへのポインタが返されます。 `free` するとチャンクに解放されたことを書いておきます。
...
| フィールド名 | 説明 |
| :-: | --- |
| prev_size | 一つ前のチャンクのサイズ。 |
| size | このチャンクのサイズ。下位 3bit はフラグとして使われる。 |
| NON_MAIN_ARENA | `main_arena` であるかどうか |
| IS_MMAPED | メモリが mmap で確保されたか |
...
より詳細な malloc / free 関数の動作は別途記事にしようと思います。とりあえずこの知識で攻撃が理解出来ると思うので進めましょう。
...
Hitcon CTF 2016 で出題された House of Orange

House Of XXX シリーズは本当に大量にあって説明しきれないのでリスト形式で紹介します。
- https://dl.packetstormsecurity.net/papers/attack/MallocMaleficarum.txt

| 攻撃名 | glibc | 説明 |
| --- | --- | --- |
| House of Prime | < 2.4 | アリーナの `max_fast` を書き換える |
| House of Corrosion | 2.26 < glibc < 2.30 | グローバル変数の `global_max_fast` を書き換える |
| House of Mind | < 2.11 | チャンクの `NON_MAIN_ARENA` |
| House of Orange | < 2.26 | top chunk のサイズを縮める unsortedbin `_IO_list_all` abort() すると _IO_flush_all_lockp() が呼び出される |
| House of Rabbit | < 2.28 | |
| House of Force | | top_chunk のサイズ |
| House of Lore | ? |  |
| House of Spirit | | |
| House of Einherjar | | チャンクの prev_inuse Hiroki Matsukuma |
